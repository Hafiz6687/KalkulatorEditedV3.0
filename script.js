...
// =========================================================
// PENAMBAHBAIKAN KHAS: KAWALAN POP-UP UNTUK FLOW 'SAMBUNG DRAF'
// =========================================================
...
window.urusPertukaranMenu = function(modDestinasi, fungsiCallback) {
    ...
    // --- TAMBAHAN BARU: Logik sasaran flow "Sambung Draf -> Klik Senarai Rekod" ---
    ...
    if (window.isSambungDrafAktif && modDestinasi === 'REKOD') {
...
// =========================================================
// PENAMBAHBAIKAN KHAS: KAWALAN POP-UP & ENJIN REBIND UNTUK FLOW 'SAMBUNG DRAF' 
// =========================================================

// 1. KEMASKINI FUNGSI BUKA DRAF (Menyambung semula Event Listener & Hidupkan flag)
window.bukaDraf = function(e) {
    // --- TAMBAHAN BARU: Hidupkan flag sambung draf ---
    window.isSambungDrafAktif = true; 
    // --------------------------------------------------
    ...
    // 3. Kembalikan kad-kad kalkulator asal & REBIND KLIK BUTANG
    let kadContainer = wrapper.querySelector('.draf-kad-container');
    if (kadContainer) {
        while(kadContainer.firstChild) {
            let kad = kadContainer.firstChild;
            ...
            // --- PENAMBAHBAIKAN PENTING: REBIND EVENT LISTENER ---
            kad.querySelectorAll('button').forEach(b => {
                let actionFunc = b.getAttribute('data-action-func');
                
                // A. Rebind Butang Kira & Reset Biasa
                if (actionFunc) {
                    let funcName = actionFunc.replace(/\(.*?\)/, '').trim();
                    let newBtn = b.cloneNode(true);
                    b.parentNode.replaceChild(newBtn, b);
                    newBtn.addEventListener('click', function(evt) {
                        activeCardContext = kad;
                        try { if (typeof window[funcName] === 'function') window[funcName](evt); } finally { activeCardContext = null; }
                    });
                }
                
                // B. Rebind Butang Kira (Mod 18A Khas) - Tiada action-func
                if (b.innerText.includes('Mod 18A')) {
                    let newBtn = b.cloneNode(true);
                    b.parentNode.replaceChild(newBtn, b);
                    newBtn.addEventListener('click', function(evt) {
                        let tempContext = activeCardContext;
                        activeCardContext = kad;
                        try {
                            let tId = kad.getAttribute('data-template-id');
                            let hariBulanInput = kad.querySelector('.hari-bulan-18a');
                            let hariBulan = hariBulanInput ? (Number(hariBulanInput.value) || 26) : (Number(window.globalHariBulan18A) || 26); 
                            
                            if (tId === 'orp') calculateORP(evt, hariBulan);
                            else if (tId === 'baki') calculateBakiUpah(evt);
                            else if (tId === 'otBiasa') calculateOTBiasa(evt, hariBulan);
                            else if (tId === 'lewat') calculateLewat(evt, hariBulan);
                            else if (tId === 'otRehat') calculateOTRH(evt, hariBulan);
                            else if (tId === 'otKelepasan') calculateOTPH(evt, hariBulan);
                            else if (tId === 'rehatKurang') calculateHariRehat(evt, hariBulan);
                            else if (tId === 'rehatLebih') calculateHariRehatLebih(evt, hariBulan);
                            else if (tId === 'kelepasan') calculatePH(evt, hariBulan);
                            else if (tId === 'cutiTahunan') calculateCutiTahunan(evt, hariBulan);
                            else if (tId === 'cutiSakit') calculateCutiSakit(evt, hariBulan);
                            else if (tId === 'sec18A') calculate18ANew(evt);
                        } finally {
                            activeCardContext = tempContext;
                        }
                    });
                ...
                // C. Rebind Butang Tutup Kad (X)
                if (b.classList.contains('close-card-btn')) {
                    let newBtn = b.cloneNode(true);
                    b.parentNode.replaceChild(newBtn, b);
                    newBtn.onclick = function() {
                        ...
                        // Kemaskini Jadual Rumusan
                        let bRumusan = document.getElementById('badanJadualRumusan');
                        if (bRumusan && tId) {
                            let mSasaran = {
                            ...
                            let sId = mSasaran[tId];
                            if (sId) {
                                bRumusan.querySelectorAll('tr').forEach(tr => {
                                    let sel = tr.querySelector('select');
                                    if (sel) {
                                        let nSel = sel.value;
                                        if (Array.isArray(sId) ? sId.includes(nSel) : nSel === sId) tr.remove(); 
                                    }
                                });
                                if (typeof kiraJumlahKeseluruhanRumusan === 'function') kiraJumlahKeseluruhanRumusan(); 
                            }
                        ...
                        let kadTinggal = document.querySelectorAll('.calculator-card:not(.hidden-template):not(.rumusan-card)');
                        if (kadTinggal.length === 0) {
                            let rCard = document.querySelector('.rumusan-card');
